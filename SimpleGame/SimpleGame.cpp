...
void hud() {
    ...
    text(left,top-objectiveRows*32.f-4,L"WASD 이동  E 대화  R 재시작  T 사냥터");
...
void down(unsigned char key,int,int) {
    ...
    if(key=='t'){
        ...
        SetWindowTextW(WindowFromDC(wglGetCurrentDC()),firstLevelActive?L"달빛마을 - 잔향의 숲":L"달빛마을 - 저녁 종");
        return;
    }
...
int main(int argc,char** argv) {
    ...
    SetWindowTextW(WindowFromDC(wglGetCurrentDC()),L"달빛마을 - 잔향의 숲");
