...
namespace FirstLevel {
    namespace {
        ...
        struct StaticMesh {
            GLuint buffer = 0;
            int count = 0;
            ~StaticMesh() { if (buffer)glDeleteBuffers(1, &buffer); }
            void Upload(const std::vector<Renderer::ModelVertex>& vertices) {
                if (!buffer)glGenBuffers(1, &buffer);
                count = int(vertices.size());
                glBindBuffer(GL_ARRAY_BUFFER, buffer);
                glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(Renderer::ModelVertex), vertices.data(), GL_STATIC_DRAW);
            }
        };
        struct State {
            ModelCache models;
            ...
            std::array<StaticMesh, 3> terrain;
            StaticMesh minimap;
        ...
        void cacheTerrain(State& s) {
            using Vertex = Renderer::ModelVertex;
            std::array<std::vector<Vertex>, 3> ground;
            std::vector<Vertex> map;
            auto quad = [](std::vector<Vertex>& vertices, Point a, Point b, Point c, Point d, float r, float g, float blue) {
                for (Point p : {a, b, c, a, c, d})vertices.push_back({ p.x,p.y,r,g,blue,1 });
                };
            for (int y = 0; y < Size; ++y)for (int x = 0; x < Size; ++x) {
                Tile t = s.tiles[index(x, y)]; Point v = iso({ x + .5f,y + .5f });
                int group = t == Water ? 2 : t == Path ? 1 : 0;
                quad(ground[group], { v.x,v.y + 21 }, { v.x + 42,v.y }, { v.x,v.y - 21 }, { v.x - 42,v.y },
                    t == Water ? .15f : .28f, t == Water ? .39f : .43f, t == Water ? .48f : .28f);
                float mx = float(x), my = float(Size - 1 - y); bool free = open(s, x, y);
                quad(map, { mx,my }, { mx + 1,my }, { mx + 1,my + 1 }, { mx,my + 1 }, free ? .34f : .12f, free ? .45f : .20f, free ? .35f : .24f);
            }
            for (int i = 0; i < 3; ++i)s.terrain[i].Upload(ground[i]);
            s.minimap.Upload(map);
        }
        ...
        bool lineClear(const State& s, Point a, Point b) {
            ...
            for (int i = 0; i <= steps; ++i) {
                float t = float(i) / steps; Point p = { a.x + (b.x - a.x) * t,a.y + (b.y - a.y) * t };
        ...
        void flood(State& s, int source) {
            ...
            while (!q.empty()) {
                int id = q.front(); q.pop(); int x = id % Size, y = id / Size;
        ...
        void checkComplete(State& s) {
            if (!s.complete && s.level >= TargetLevel && s.strength + s.vitality + s.guard >= RequiredAllocation && s.pickups > 0) {
                s.complete = true;
        ...
        void generate(State& s) {
            ...
            for (int i = 0; i < 60; ++i) {
                int cx = 2 + random() % 36, cy = 2 + random() % 36, rx = 1 + random() % 3, ry = 1 + random() % 3;
            ...
            for (Point p : candidates) {
                bool free = true; for (const auto& e : s.enemies)if (distance(e.p, p) < 2.5f)free = false;
        ...
        void attack(State& s) {
            ...
            for (auto& e : s.enemies) {
                if (e.hp <= 0)continue; float d = distance(e.p, s.player);
                ...
                if (d < 1.8f && (d < .5f || dot / d>.20f) && lineClear(s, s.player, e.p)) {
                    ...
                    if (e.hp <= 0) {
                        ++s.kills; addXp(s, 20); s.drops.push_back({ e.p,s.kills % 3 == 0 ? Potion : Shard });
        ...
        void allocate(State& s, unsigned char key) {
            ...
            if (key == '1') { ++s.strength; }
            else if (key == '2') { ++s.vitality; s.hp += 12; }
            else if (key == '3') { ++s.guard; }
            else return;
    ...
    void Update(float dt) {
        ...
        for (auto& e : s.enemies) {
            ...
            if (e.windup > 0) {
                e.windup -= dt; if (e.windup <= 0) {
                    e.cooldown = 1.3f;
                    if (d < 1.35f && distance(s.player, Camp)>2.8f && lineClear(s, e.p, s.player) && s.invulnerable <= 0) {
                        int damage = std::max(1, 12 - s.defense()); s.hp -= damage; s.invulnerable = .55f;
                ...
                if (best > 0) {
                    target = e.p; for (auto offset : { std::pair<int,int>{1,0},{-1,0},{0,1},{0,-1} }) {
    ...
    void Draw(Renderer& r, int width, int height) {
        ...
        const int materials[] = { 2,5,8 };
        for (int i = 0; i < 3; ++i) {
            const auto& mesh = s.terrain[i];
            if (mesh.count)r.DrawCachedModel(mesh.buffer, mesh.count, -camera.x, -camera.y, 1, 0, materials[i]);
        }
        r.SetMaterial(0);
        struct Object { Point p; int model; float phase, flash; }; std::vector<Object> objects;
        for (int y = 0; y < Size; ++y)for (int x = 0; x < Size; ++x) {
            Tile t = s.tiles[index(x, y)]; Point p = { x + .5f,y + .5f };
        ...
        for (const auto& e : s.enemies)if (e.hp > 0 && visible(e.p) && distance(s.player, e.p) < 8) {
            Point v = screen(e.p, camera);
        ...
        if (s.slash > 0) {
            Point center = screen(s.player, camera); float angle = std::atan2(s.slashDirection.y, s.slashDirection.x);
            for (int i = 0; i < 16; ++i) {
                float a = angle - 1.1f + i * 2.2f / 16, b = a + 2.2f / 16;
        ...
        r.DrawCachedModel(s.minimap.buffer, s.minimap.count, mx, my, unit);
        ...
        if (s.statsOpen) {
            std::wstring stats = L"능력치  ·  남은 포인트 " + std::to_wstring(s.points) +
